...
export class ZeroXCollector extends QuoteCollector {
  ...
  start(): void {
    ...
    if (pairs.length === 0) {
      this.logger.warn("ARB_0X_PAIRS empty; no 0x pairs");
      return;
    }
    ...
    this.intervalId = setInterval(async () => {
      for (const pair of pairs) {
        ...
        try {
          ...
          this.logger.warn({ err }, "0x quote failed");
        }
      }
    }, 4000);
  }
...
function parsePairs(): ZeroXPair[] {
  const raw = process.env.ARB_0X_PAIRS;
  if (!raw) return [];
  try {
    const parsed = JSON.parse(raw) as ZeroXPair[];
